...
t=netcdf.netcdf_file('topoin.nc')
topography = t.variables['topo']
t = []
...
while elapsed_time < run_time:
    ...
    os.chdir('/Users/blynch/Research/Working_Directories/wrf_hydro/Andes/discharge_2013') #Location of discharge maps
    ...
    f=netcdf.netcdf_file(discharge_filename)
    discharge = f.variables['flow']
    f = []
    for i in range(0,1990):
        for j in range(0,1990):
            f.append(discharge[i][j])
    water__volume_flux_in = np.array(f,dtype=float)  #making input file into an array so it can be read by landlab

    water__volume_flux_in = mg.add_field('node', 'water__volume_flux_in', water__volume_flux_in) #create the field 


    os.chdir('/Users/blynch/Research/Working_Directories/landlab')

    _ = fr.route_flow()
    #_ = sp.erode(mg, dt, node_elevs='topographic__elevation', node_drainage_areas='drainage_area', flow_receiver='flow_receiver', node_order_upstream='upstream_ID_order', slopes_at_nodes= 'topographic__steepest_slope', Q_if_used='water__volume_flux_in')
    _ = sp.erode(mg, dt, node_elevs='topographic__elevation', node_drainage_areas='drainage_area', Q_if_used='water__volume_flux_in')
        #add uplift
    mg.at_node['topographic__elevation'][mg.core_nodes] +=uplift_rate
    
    for i in xrange(nt):
        _ = lin_diffuse.diffuse(dt)
    
    elapsed_time += dt
    
    #diffusion driver loop
    #for i in xrange(nt): # loop nt times
    #perform hillslope diffusion,swap between linear (much faster) and nonlinear diffusion
   # _ = lin_diffuse.diffuse(dt) #linear diffusion
        #_ = diffuse.diffuse(mg, i*dt) #nonlinear diffusion
...
write_netcdf('discharge_out.nc', mg, names='water__volume_flux_in')
write_netcdf('discharge_out2.nc', mg, names='water__volume_flux')
write_netcdf('drainage_area_out.nc', mg, names='drainage_area')
write_netcdf('stream_power_erosion.nc', mg, names='stream_power_erosion')
write_netcdf('slopes.nc', mg, names='topographic__steepest_slope')
